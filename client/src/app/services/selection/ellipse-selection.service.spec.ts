...
describe('EllipseSelectionService', () => {
	...
	beforeEach(() => {
		...
		spyOn<any>(service, 'drawEllipse').and.callThrough();
		spyOn<any>(service, 'drawCircle').and.callThrough();
		...
		spyOn(baseCtxStub, 'drawImage').and.callFake(() => {
			return;
		});
	...
	it(' updatePreview should not call drawEllipse if shift is down', () => {
		service['shiftDown'] = true;
		service['updatePreview']();
		expect(service['drawEllipse']).not.toHaveBeenCalled();
	});
	...
	it(' updatePreview should not call drawCircle if shift is up', () => {
		service['shiftDown'] = false;
		service['updatePreview']();
		expect(service['drawCircle']).not.toHaveBeenCalled();
	});
	...
	it(' onMouseUp should call selectRectangle', () => {
		service.isDrawing = true;
		service.onMouseUp(Global.MOUSE_EVENTS[MouseEnum.PAGE_REF]);
	...
	it('drawEllipse should call all the appropriate methods', () => {
		service.isDrawing = true;
		service['drawEllipse'](previewCtxStub, service.origin, service['target']);
		expect(previewCtxStub.ellipse).toHaveBeenCalled();
		expect(baseCtxStub.getImageData).toHaveBeenCalled();
	});

	it('drawCircle should call all the appropriate methods', () => {
		service.isDrawing = true;
		service['drawCircle'](previewCtxStub, service.origin, service['target']);
		expect(previewCtxStub.ellipse).toHaveBeenCalled();
		expect(baseCtxStub.getImageData).toHaveBeenCalled();
	});

	it('drawEllipse should set the deltas to 1 if they have a value of 0', () => {
	...
	it('drawEllipse should not set the deltas to 1 if they do not have a value of 0', () => {
		service['drawEllipse'](previewCtxStub, service.origin, service['target']);
	...
	it('drawCircle should set deltaX to 1 if it has a value of 0', () => {
		service['target'] = service.origin = Global.DEFAULT_VECTOR;
		service['drawCircle'](previewCtxStub, service.origin, service['target']);
	...
	it('drawCircle should not set the deltas to 1 if they do not have a value of 0', () => {
		service['drawCircle'](previewCtxStub, service.origin, service['target']);
	...
	it('selectRectangle should change the box size and place it', () => {
	...
	it('limitRectanglePosition should cap the target position to the size of the canvas', () => {
	...
	it('limitRectanglePosition should not change anything when target is on canvas', () => {
		// tslint:disable:no-string-literal
		service['limitRectanglePosition'](previewCtxStub, service.origin, service['target']);
	...
	it('limitSquarePosition should change the deltas if square would be out of bounds', () => {
	...
	it('redrawSelection should change origin to position parameter', () => {
		service.origin = Global.TEST_TARGET_VECTOR;
		service.redrawSelection(Global.TEST_VECTOR, 0, Global.TEST_VECTOR, service.selection);
	...
	it('redrawSelection should not do anything if canvas does not exist', () => {
		...
		service.redrawSelection(Global.DEFAULT_VECTOR, 0, Global.TEST_VECTOR, service.selection);
