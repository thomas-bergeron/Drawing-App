...
(function (global, factory) {
	...
	function report(_ref) {
		...
		if (noConflictsCount > 0) {
			console.info(
				'%cNo conflict'.concat(noConflictsCount > 1 ? 's' : '', ' found with ').concat(noConflictsCount == 1 ? 'this' : 'these', ':'),
		...
		if (timeOutCount > 0) {
			console.info(
				'%cLeftovers--we timed out before collecting test results for '.concat(timeOutCount == 1 ? 'this' : 'these', ':'),
	...
	function conflictDetection() {
		...
		if (testCount === 0) {
			...
			console.info(
				'\t%c'.concat(resultsCollectionMaxWaitAttr, '%c: milliseconds to wait for the browser to accumulate test results before giving up.'),
				'font-weight: bold;',
				'font-size: normal;',
			);
			pollUntil({
